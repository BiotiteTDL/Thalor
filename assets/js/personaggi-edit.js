(function(){
  ...
  function mergeOnlineSheetItems(baseItems,rows){
    ...
    (Array.isArray(rows)?rows:[]).forEach(row=>{
      ...
      if(item){
        ...
        // Il registro personaggi è la fonte più ricca per descrizioni/eventi.
        // Le schede online servono solo come recupero/fallback: non devono mai
        // sovrascrivere longHtml/eventsHtml/desc appena importati o modificati.
        const merged=Object.assign({},item,current,{
          img:current.img||item.img,
          desc:current.desc||item.desc,
          longDesc:current.longDesc||item.longDesc,
          events:current.events||item.events,
          longHtml:current.longHtml||item.longHtml||'',
          eventsHtml:current.eventsHtml||item.eventsHtml||''
        });
        map.set(item.slug,merged);
      }
    });
  ...
  function sameEditedText(a,b){return String(a||'').replace(/\r/g,'').trim()===String(b||'').replace(/\r/g,'').trim();}
  function editedTextToHtml(text){return paragraphs(String(text||''));}
  function syncSheetContentFromItem(item,data){
    if(!item||!data)return data;
    data.meta=data.meta||{};
    data.identity=data.identity||{};
    data.portrait=data.portrait||{};
    data.narrative=data.narrative||{};
    data.meta.slug=item.slug;
    data.meta.permissionRole=item.slug;
    data.meta.characterRole=item.slug;
    data.meta.profileUrl=`dettaglio.html?id=${item.slug}`;
    data.identity.name=item.name||data.identity.name||item.slug;
    if(item.playerName!==undefined)data.identity.player=item.playerName||'';
    data.portrait.image=toSheetImg(item.img)||data.portrait.image||'';
    data.portrait.quote=item.desc||data.portrait.quote||'';
    data.narrative.diary=item.longDesc||htmlToEditText(item.longHtml)||data.narrative.diary||'';
    return data;
  }
  ...
  function collectForm(form,item,oldSlug){
    const fd=new FormData(form);
    const previousLongHtml=item.longHtml||'';
    const previousEventsHtml=item.eventsHtml||'';
    const previousLongText=item.longDesc||htmlToEditText(previousLongHtml)||'';
    const previousEventsText=item.events||htmlToEditText(previousEventsHtml)||'';
    const nextLongText=String(fd.get('longDesc')||'');
    const nextEventsText=String(fd.get('events')||'');

    item.type=fd.get('type')||'pg';
    item.name=fd.get('name')||'Nuovo personaggio';
    const desired=slugify(fd.get('slug')||item.name);
    item.slug=(oldSlug&&desired!==oldSlug&&state.items.some(i=>i!==item&&i.slug===desired))?uniqueSlug(desired):desired;
    item.playerName=fd.get('playerName')||'';
    ensureRole(item);
    makeLinks(item);
    item.desc=fd.get('desc')||'';
    item.longDesc=nextLongText;
    item.events=nextEventsText;
    // Se il master modifica solo la descrizione breve, preserviamo l'HTML ricco
    // già importato/esistente. Prima questo veniva azzerato e al refresh sembrava
    // che descrizione/eventi di Elia sparissero o tornassero vecchi.
    item.longHtml=sameEditedText(nextLongText,previousLongText)?previousLongHtml:editedTextToHtml(nextLongText);
    item.eventsHtml=sameEditedText(nextEventsText,previousEventsText)?previousEventsHtml:editedTextToHtml(nextEventsText);
    item.img=fd.get('img')||'';
    return item;
  }
  ...
  async function refreshPersonaggiList(force=false){
    ...
    try{
      ...
      // Prima leggiamo i dati pubblici online, poi controlliamo i poteri.
      // Così un browser fresh non resta bloccato su auth/init prima del render pubblico.
  ...
  if($('#personaggioDetailApp')){ initDetail().then(()=>canMaster().then(m=>{state.master=m; const app=$('#personaggioDetailApp'); const qs=new URLSearchParams(location.search); const id=qs.get('id')||qs.get('character')||qs.get('slug')||''; const item=(state.items||[]).find(x=>x.slug===id); if(state.master&&item&&app&&!$('#personaggiMasterDock'))renderDetailDock(item,app); }).catch(()=>{})); }
})();
