(function(){
  ...
  function syncSheetFromItem(item,data){
    ensureRole(item);
    const sheet=data&&typeof data==='object'?data:blankSheet(item);
    sheet.meta=sheet.meta||{};
    sheet.meta.slug=item.slug;
    sheet.meta.permissionRole=item.slug;
    sheet.meta.characterRole=item.slug;
    sheet.meta.profileUrl=`dettaglio.html?id=${item.slug}`;
    sheet.meta.subtitle=item.type==='png'?'PNG':(sheet.meta.subtitle||'Personaggio');
    sheet.meta.theme=item.type==='png'?'necrotic':(sheet.meta.theme||'default');
    sheet.identity=sheet.identity||{};
    sheet.identity.name=item.name||sheet.identity.name||'Nuovo personaggio';
    sheet.identity.player=item.playerName||sheet.identity.player||'';
    sheet.portrait=sheet.portrait||{};
    if(item.img) sheet.portrait.image=toSheetImg(item.img);
    sheet.portrait.alt=item.name||sheet.portrait.alt||'';
    if(item.desc!==undefined) sheet.portrait.quote=item.desc||'';
    sheet.narrative=sheet.narrative||{};
    if(item.longDesc!==undefined && item.longDesc) sheet.narrative.diary=item.longDesc;
    try{localStorage.setItem(sheetKey(item.slug),JSON.stringify(sheet));}catch(e){console.warn('Cache locale scheda non aggiornata:',e);}
    return sheet;
  }
