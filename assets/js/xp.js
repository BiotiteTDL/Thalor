...
(async function start(){
  ...
  try{if(authAvailable()&&window.ThalorAuth.state.configured&&navigator.onLine!==false){const online=await window.ThalorAuth.loadCharacter(slug,null);if(meaningful(online)){data=online;freshLoaded=true;localStorage.setItem(storageKey,JSON.stringify(normalize(data)));}}}
