...
function editDeniedMessage(){return 'Accesso richiesto: puoi modificare solo il personaggio assegnato, oppure tutto se sei Master.';}
...
const blank={condition:{name:'Custom',active:'Sì',duration:'',strMod:0,desMod:0,cosMod:0,intMod:0,sagMod:0,carMod:0,attackMod:0,damageMod:0,acMod:0,saveMod:0,skillMod:0,notes:''},bonus:{target:'ac',save:'all',type:'senza tipo',value:0,source:'',note:''},attack:{name:'Nuovo attacco',attackAbility:'FOR',damageAbility:'FOR',damageDice:'',critical:'',range:'',type:'',magicBonus:0,attackMisc:0,damageMagic:0,damageMisc:0,bonuses:[],notes:''},defense:{name:'Nuova protezione',bonus:0,type:'armatura',maxDex:'',checkPenalty:0,spellFailure:0,weight:'',bonuses:[],notes:''},skill:{name:'Nuova abilità',ability:'INT',standardAbility:'INT',ranks:0,misc:0,classSkill:'No',armorApplies:false,notes:''},feat:{name:'Nuovo talento',description:'',source:''},feature:{name:'Nuova capacità / tratto',description:'',source:''},language:{name:'Nuovo linguaggio',notes:''},spell:{name:'Nuovo incantesimo/potere',school:'',castingTime:'',time:'',target:'',duration:'',description:''},spellCircle:{level:1,className:'',ability:'',misc:0,slots:0,prepared:0,used:0,spells:[]},classLevel:{name:'Nuova classe',level:1,notes:''},item:{name:'Nuovo oggetto',qty:1,weight:'',equipped:'No',bonuses:[],notes:''},inventorySection:{name:'Nuova sezione',notes:'',items:[]},companion:{kind:'Famiglio',notes:'',sheet:null}};
...
function globalBonusTotals(d){const buckets={ac:[],touch:[],flat:[],attack:[],damage:[],initiative:[],grapple:[],skill:[],fortitude:[],reflex:[],will:[],FOR:[],DES:[],COS:[],INT:[],SAG:[],CAR:[]};function add(b){b=Object.assign({},blank.bonus,b);if(b.target==='save'){if(!b.save||b.save==='all'){buckets.fortitude.push(b);buckets.reflex.push(b);buckets.will.push(b)}else if(buckets[b.save])buckets[b.save].push(b);return;} if(buckets[b.target])buckets[b.target].push(b);} (d.defenses||[]).forEach(x=>normalizeBonusList(x.bonuses).forEach(add));(d.combat?.tempBonuses||[]).forEach(add);(d.inventorySections||[]).forEach(sec=>(sec.items||[]).forEach(it=>{if(String(it.equipped||'No').toLowerCase().startsWith('s')) normalizeBonusList(it.bonuses).forEach(add);}));AB.forEach(([k])=>normalizeBonusList(d.abilities?.[k]?.bonuses).forEach(b=>add(Object.assign({},b,{target:k}))));return Object.fromEntries(Object.entries(buckets).map(([k,v])=>[k,stackBonuses(v)]));}
function bonusRows(path,list){list=normalizeBonusList(list);let count=list.length;let abilityMode=path.startsWith('abilities.');return `<details class="bonus-editor edit-only" data-detail-id="bonus-${esc(path)}"><summary><span>Bonus avanzati</span><strong>${count}</strong></summary><div class="bonus-editor-body">${list.map((b,i)=>{let target=b.target||'ac',showSave=target==='save';return `<div class="bonus-row ${abilityMode?'ability-bonus-row':''}" data-bonus-row><label class="bonus-target-field ${abilityMode?'hidden-field':''}">Applica a ${bonusTargetSelect(`${path}.${i}.target`,target)}</label><label class="bonus-save-choice ${showSave?'':'hidden-field'}" data-bonus-save-wrap>Quale TS ${bonusSaveSelect(`${path}.${i}.save`,b.save)}</label><label>Tipo ${bonusTypeSelect(`${path}.${i}.type`,b.type)}</label><label>Valore ${inp(`${path}.${i}.value`,b.value,'number')}</label><label class="bonus-source-field">Fonte ${inp(`${path}.${i}.source`,b.source||'')}</label><label class="bonus-note-field">Note ${inp(`${path}.${i}.note`,b.note||'')}</label><button class="row-del" data-del="${path}.${i}" title="Elimina bonus">×</button></div>`}).join('')||`<p class="section-mini-note">Nessun bonus aggiunto.</p>`}<button class="mini-add" data-add="${path}" data-kind="bonus" hidden>+ Bonus avanzato</button></div></details>`}
function bonusTargetLabel(k,save){
  const map={ac:'CA',touch:'CA contatto',flat:'CA sprovvista',attack:'TPC',damage:'Danni',initiative:'Iniziativa',grapple:'Lotta',skill:'Abilità',FOR:'FOR',DES:'DES',COS:'COS',INT:'INT',SAG:'SAG',CAR:'CAR'};
  if(k==='save') return save&&save!=='all' ? ({fortitude:'Tempra',reflex:'Riflessi',will:'Volontà'}[save]||save) : 'TS';
  return map[k]||k||'Bonus';
}
function bonusTypeLabel(t){return (BONUS_TYPES.find(x=>x[0]===t)||['',t||'senza tipo'])[1]}
function bonusTip(b, forcedTarget){
  b=Object.assign({},blank.bonus,b);
  const targetText=forcedTarget||bonusTargetLabel(b.target,b.save);
  const bits=[`Applica a: ${targetText}`,`Valore: ${sign(b.value)}`,`Tipo: ${bonusTypeLabel(b.type)}`];
  if(String(b.source||'').trim())bits.push(`Fonte: ${b.source}`);
  if(String(b.note||'').trim())bits.push(`Note: ${b.note}`);
  return bits.join('\n');
}
...
function hero(d){let can=sheetCanEdit();return `<section class="dynamic-hero sheet-theme-panel"><div class="dynamic-crest">${esc(d.meta?.crest||'✦')}</div><div class="hero-main"><span class="tag">${isCompanion?'Scheda secondaria collegata':'Scheda personaggio — WorldAnvil style'}</span><h1 class="section-title">${inp('identity.name',d.identity?.name||'')}</h1><p class="section-note">${esc(d.meta?.subtitle||d.meta?.status||'')}</p><div class="actions sheet-actions"><a class="button primary-action" href="${esc(isCompanion?`scheda-${slug}.html`:(d.meta?.profileUrl||'../personaggi.html'))}">${isCompanion?'Torna alla scheda principale':'Torna al profilo'}</a><a class="button ghost-button primary-action" href="../auth.html">Accesso</a><button class="button primary-action" id="editSheet" type="button" ${can?'':'disabled'}>${can?'Modifica':'Solo lettura'}</button><button class="button save-button primary-action" id="saveSheet" type="button" hidden>Salva ${authAvailable()&&window.ThalorAuth.state.configured?'online':'nel browser'}</button><button class="button ghost-button primary-action" id="printSheet" type="button">Stampa/PDF</button><details class="sheet-tools"><summary class="button ghost-button">Strumenti</summary><div class="sheet-tools-grid"><button class="button ghost-button" id="exportSheet" type="button">Esporta JSON</button><button class="button ghost-button" id="copySheet" type="button">Copia backup</button><button class="button ghost-button" id="exportCompendium" type="button">Esporta compendio</button><label class="button ghost-button import-label ${can?'':'disabled-tool'}">Importa JSON<input id="importSheet" type="file" accept="application/json" ${can?'':'disabled'} hidden></label><button class="button ghost-button danger-action" id="resetSheet" type="button" ${can?'':'disabled'}>Ripristina JSON</button></div></details></div><p class="sheet-status auth-status-line">${esc(authStatusText())}</p><p class="sheet-status" id="localStatus">${can?'Pronto.':'Solo lettura: accedi con il proprietario o Master.'}</p></div></section>`}
...
function abilities(d){let gb=globalBonusTotals(d);return `<article class="panel sheet-theme-panel abilities-panel"><h2>Caratteristiche</h2><p class="section-mini-note">I bonus temporanei tipizzati vengono sommati come layer separato. I bonus dello stesso tipo sulla stessa caratteristica non si sommano, salvo tipi cumulabili.</p><div class="ability-grid">${AB.map(([k,l])=>{let a=d.abilities?.[k]||{score:10,temp:0},baseMod=mod(a.score,a.temp),eff=abilityScoreEffective(d,k),effMod=abilityMod(d,k);return `<div class="ability-card"><span>${k}</span><strong>${inp(`abilities.${k}.score`,a.score,'number')}</strong><em>${readonly(sign(effMod))}</em><small>${l}</small><label>Bonus temp. semplice</label>${inp(`abilities.${k}.temp`,a.temp||0,'number')}<small>Effettivo: ${eff} · Bonus tipizzati: ${sign(gb[k]||0)}</small>${bonusSummary(a.bonuses||[],k)}${bonusRows(`abilities.${k}.bonuses`,a.bonuses||[])}</div>`}).join('')}</div></article>`}
...
function combat(d){let c=calc(d),cb=d.combat||{},hp=hpStatus(d);return `<article class="panel sheet-theme-panel"><h2>Combattimento</h2><div class="computed-row"><div><span>CA</span><strong>${c.ca}</strong></div><div><span>Contatto</span><strong>${c.touch}</strong></div><div><span>Sprovvista</span><strong>${c.flat}</strong></div><div><span>Iniziativa</span><strong>${sign(c.init)}</strong></div><div><span>Lotta</span><strong>${sign(c.grapple)}</strong></div></div><div class="formula-box"><strong>CA automatica:</strong> Base 10 + Protezioni/Difese + DES utilizzabile + bonus tipizzati + condizioni. I vecchi componenti CA manuali non sono più modificabili da qui: usa Difese / Protezioni e Bonus avanzati.</div><h3>Punti ferita e base</h3><div class="computed-row hp-status-row"><div><span>Stato PF</span><strong>${esc(hp.status)}</strong></div><div><span>PF effettivi con temp.</span><strong>${hp.effective}</strong></div></div><div class="sheet-stat-grid combat-grid">${[['hpCurrent','PF attuali'],['hpMax','PF massimi'],['hpTemp','PF temporanei'],['nonlethal','Danni non letali'],['stable','Stabile?'],['speed','Velocità'],['bab','BAB'],['initiativeMisc','Iniziativa vari'],['grappleMisc','Lotta vari'],['resistances','Resistenze/Note']].map(([k,l])=>`<div class="sheet-stat"><span>${l}</span>${k==='stable'?yesNo('combat.'+k,cb[k]||'No'):inp('combat.'+k,cb[k]??'',k.includes('hp')||k==='nonlethal'||k==='bab'||k.includes('Misc')?'number':'text')}</div>`).join('')}</div>${bonusRows('combat.tempBonuses',cb.tempBonuses||[])}<h3>Riepilogo difese automatiche</h3><div class="sheet-stat-grid combat-grid combat-summary-grid"><div class="sheet-stat computed-small"><span>Protezioni</span>${readonly(sign(c.pt.total))}</div><div class="sheet-stat computed-small"><span>Bonus CA tipizzati</span>${readonly(sign(c.gb.ac||0))}</div><div class="sheet-stat computed-small"><span>DES usata</span>${readonly(sign(c.maxDex))}</div><div class="sheet-stat computed-small"><span>Pen. armatura</span>${readonly(sign(c.pt.armorPenalty))}</div><div class="sheet-stat computed-small"><span>Fall. inc. arcani</span>${readonly(c.spellFailure+'%')}</div></div></article>`}
function saves(d){let c=calc(d);return `<article class="panel sheet-theme-panel"><h2>Tiri salvezza</h2><table class="sheet-table"><thead><tr><th>TS</th><th>Totale</th><th>Car.</th><th>Base</th><th>Magia</th><th>Vari</th><th>Bonus tip.</th><th>Cond.</th></tr></thead><tbody>${Object.entries(d.saves||{}).map(([k,s])=>{let typed=num(c.gb[k]||0);let total=num(s.base)+abilityMod(d,s.ability)+num(s.magic)+num(s.misc)+typed+c.ct.saveMod;return `<tr><td>${esc(s.label)}</td><td>${readonly(sign(total))}</td><td>${sel(`saves.${k}.ability`,s.ability,s.ability)}</td><td>${inp(`saves.${k}.base`,s.base,'number')}</td><td>${inp(`saves.${k}.magic`,s.magic,'number')}</td><td>${inp(`saves.${k}.misc`,s.misc,'number')}</td><td>${readonly(sign(typed))}</td><td>${readonly(sign(c.ct.saveMod))}</td></tr>`}).join('')}</tbody></table></article>`}
function attacks(d){let c=calc(d);let body=`<div class="section-head"><button class="mini-add" data-add="attacks" data-kind="attack" hidden>+ Attacco</button></div><p class="section-mini-note">Il bonus magico arma si applica automaticamente sia al tiro per colpire sia ai danni. Per altri modificatori usa i bonus avanzati: i bonus dello stesso tipo sulla stessa voce non si sommano.</p><div class="attack-card-list">${(d.attacks||[]).map((a,i)=>{let localAtk=stackBonuses(normalizeBonusList(a.bonuses).filter(b=>b.target==='attack'));let localDmg=stackBonuses(normalizeBonusList(a.bonuses).filter(b=>b.target==='damage'));let magic=num(a.magicBonus);let atk=num(d.combat?.bab)+abilityMod(d,a.attackAbility)+magic+num(c.gb.attack)+localAtk+c.ct.attackMod;let dmg=magic+abilityMod(d,a.damageAbility)+num(c.gb.damage)+localDmg+c.ct.damageMod;return `<article class="attack-card"><div class="attack-card-main"><div class="attack-name">${inp(`attacks.${i}.name`,a.name)}</div><div class="attack-result"><span>TPC</span><strong>${sign(atk)}</strong></div><div class="attack-result"><span>Danni</span><strong>${esc(a.damageDice||'')} ${sign(dmg)}</strong></div><button class="row-del edit-only attack-delete" data-del="attacks.${i}">×</button></div><div class="attack-edit-grid edit-only"><label>Dadi danno ${inp(`attacks.${i}.damageDice`,a.damageDice||'')}</label><label>Car. attacco ${sel(`attacks.${i}.attackAbility`,a.attackAbility,'FOR')}</label><label>Car. danno ${sel(`attacks.${i}.damageAbility`,a.damageAbility,'FOR')}</label><label>Bonus magico arma ${inp(`attacks.${i}.magicBonus`,a.magicBonus,'number')}</label><label>Critico ${inp(`attacks.${i}.critical`,a.critical)}</label><label>Tipo ${inp(`attacks.${i}.type`,a.type||'')}</label></div><div class="attack-note-line">${a.critical?`<span>Critico: ${esc(a.critical)}</span>`:''}${a.type?`<span>${esc(a.type)}</span>`:''}${bonusSummary(a.bonuses)}</div><div class="attack-notes edit-only">${area(`attacks.${i}.notes`,a.notes||'')}</div>${bonusRows(`attacks.${i}.bonuses`,a.bonuses||[])}</article>`}).join('')||`<p class="empty-row">Nessun attacco inserito.</p>`}</div>`;return section('Attacchi e tecniche',body,{wide:true,open:true})}
function defenses(d){let body=`<div class="section-head"><button class="mini-add" data-add="defenses" data-kind="defense" hidden>+ Protezione</button></div><p class="section-mini-note">Bonus CA, penalità armatura e fallimento incantesimi arcani vengono sommati automaticamente. I bonus tipizzati aggiuntivi rispettano lo stacking 3.5.</p><table class="sheet-table"><thead><tr><th>Protezione</th><th>Bonus CA</th><th>Tipo</th><th>DES max</th><th>Pen. prova</th><th>Fall. inc.</th><th>Peso</th><th>Note / bonus</th><th class="edit-only">Azioni</th></tr></thead><tbody>${(d.defenses||[]).map((r,i)=>`<tr><td>${inp(`defenses.${i}.name`,r.name)}</td><td>${inp(`defenses.${i}.bonus`,r.bonus,'number')}</td><td>${inp(`defenses.${i}.type`,r.type)}</td><td>${inp(`defenses.${i}.maxDex`,r.maxDex)}</td><td>${inp(`defenses.${i}.checkPenalty`,r.checkPenalty,'number')}</td><td>${inp(`defenses.${i}.spellFailure`,r.spellFailure,'number')}%</td><td>${inp(`defenses.${i}.weight`,r.weight)}</td><td>${area(`defenses.${i}.notes`,r.notes||'')}${bonusSummary(r.bonuses)}${bonusRows(`defenses.${i}.bonuses`,r.bonuses||[])}</td><td class="edit-only"><button class="row-del" data-del="defenses.${i}">×</button></td></tr>`).join('')}</tbody></table>`;return section('Difese / Protezioni',body,{wide:true})}
function skills(d){let c=calc(d),lvl=levelFrom(d);let body=`<div class="section-head"><button class="mini-add" data-add="skills" data-kind="skill" hidden>+ Abilità</button></div><p class="section-mini-note">Totale: gradi + caratteristica + mod vari + penalità armatura automatica + condizioni. Max gradi: classe ${lvl+3}, incrociata ${(lvl+3)/2}.</p><table class="sheet-table skill-table"><thead><tr><th>Abilità</th><th>Totale</th><th>Car.</th><th>Gradi</th><th>Max</th><th>Mod vari</th><th>Pen. arm.</th><th>Cond.</th><th>Classe</th><th>Note</th><th class="edit-only">Azioni</th></tr></thead><tbody>${(d.skills||[]).map((s,i)=>{let applies=s.armorApplies===true||String(s.armorApplies)==='Sì';let pen=applies?c.pt.armorPenalty*(String(s.name).toLowerCase()==='nuotare'?2:1):0;let condSkill=conditionSkillMod(d,s);let total=num(s.ranks)+abilityMod(d,s.ability)+num(s.misc)+pen+condSkill;let isClass=String(s.classSkill).toLowerCase().startsWith('s');let max=isClass?lvl+3:(lvl+3)/2;let warn=num(s.ranks)>max?' rank-warn':'';return `<tr class="${warn}"><td>${inp(`skills.${i}.name`,s.name)}</td><td>${readonly(sign(total))}</td><td>${sel(`skills.${i}.ability`,s.ability,s.standardAbility)}</td><td>${inp(`skills.${i}.ranks`,s.ranks,'number')}</td><td>${readonly(max)}</td><td>${inp(`skills.${i}.misc`,s.misc,'number')}</td><td>${readonly(sign(pen))}<span class="edit-only-inline">${yesNo(`skills.${i}.armorApplies`,applies?'Sì':'No')}</span></td><td>${readonly(sign(condSkill))}</td><td>${yesNo(`skills.${i}.classSkill`,isClass?'Sì':'No')}</td><td>${inp(`skills.${i}.notes`,s.notes)}</td><td class="edit-only"><button class="row-del" data-del="skills.${i}">×</button></td></tr>`}).join('')}</tbody></table>`;return section('Abilità',body,{wide:true})}
function tooltipCard(title,path,arr,kind){let body=`<div class="section-head"><button class="mini-add" data-add="${path}" data-kind="${kind}" hidden>+ Aggiungi</button></div><div class="wa-cards tooltip-cards">${(arr&&arr.length?arr:[blank[kind]]).map((x,i)=>`<div class="wa-card info-card" tabindex="0" data-tip="${esc(tipText(x))}"><h3>${inp(`${path}.${i}.name`,x.name||'','text','compendium-name')}</h3>${area(`${path}.${i}.description`,x.description||x.notes||'','edit-description')}<div class="edit-description field-note-label">Fonte / note</div>${inp(`${path}.${i}.source`,x.source||'','text','edit-description')}<button class="row-del edit-only" data-del="${path}.${i}">×</button></div>`).join('')}</div>`;return section(title,body)}
function languages(d){let body=`<div class="section-head"><button class="mini-add" data-add="languages" data-kind="language" hidden>+ Linguaggio</button></div><div class="wa-cards compact language-cards">${(d.languages&&d.languages.length?d.languages:[blank.language]).map((x,i)=>`<div class="wa-card"><h3>${inp(`languages.${i}.name`,x.name||'')}</h3>${inp(`languages.${i}.notes`,x.notes||'')}<button class="row-del edit-only" data-del="languages.${i}">×</button></div>`).join('')}</div>`;return section('Linguaggi',body)}
...
function spells(d){let c=calc(d),sc=d.spellcasting||{},sr=num(sc.casterLevel)+num(sc.srMisc),groups=sc.groups||[];let body=`<div class="spell-summary"><div><span>Fallimento inc. arcani</span><strong>${c.spellFailure}%</strong></div><div><span>Prova contro RI</span><strong>${sign(sr)}</strong></div><div><span>Livello incantatore</span><div class="spell-summary-field">${inp('spellcasting.casterLevel',sc.casterLevel,'number')}</div></div><div><span>Mod RI vari</span><div class="spell-summary-field">${inp('spellcasting.srMisc',sc.srMisc,'number')}</div></div></div><p class="section-mini-note">Le cerchie sono dinamiche: aggiungi solo quelle che servono, scegliendo numero e classe di appartenenza. Gli slot usati si possono aumentare o togliere anche senza entrare in modifica.</p><div class="spell-circle-adder edit-only"><label>Cerchia ${`<select id="newSpellCircleLevel">${spellCircleOptions(1)}</select>`}</label><label>Classe ${`<input id="newSpellCircleClass" type="text" placeholder="Es. Mago, Chierico, CdP...">`}</label><label>Inserisci ${`<select id="newSpellCirclePos"><option value="end">In fondo</option>${groups.map((g,i)=>`<option value="${i}">Prima di ${esc(spellGroupTitle(g))}</option>`).join('')}</select>`}</label><button class="mini-add" id="addSpellCircle" type="button">+ Aggiungi cerchia</button></div><div class="spell-circles">${groups.map((g,gi)=>{let dc=spellDC(d,g), used=num(g.used), slots=num(g.slots);return `<details class="spell-circle" data-spell-group="${gi}"><summary><span class="spell-circle-title">${esc(spellGroupTitle(g))}</span><span class="spell-dc-badge">${esc(dc.short)}</span><span class="spell-dc-edit edit-only" onclick="event.stopPropagation()"><span class="spell-dc-formula">${esc(dc.text)}</span><label>Cerchia</label><select data-path="spellcasting.groups.${gi}.level" disabled>${spellCircleOptions(g.level)}</select><label>Classe</label>${inp(`spellcasting.groups.${gi}.className`,g.className||'')}<label>Car.</label>${sel(`spellcasting.groups.${gi}.ability`,dc.ab,'',true)}<label>Mod CD</label>${inp(`spellcasting.groups.${gi}.misc`,dc.misc,'number')}<label>Slot</label>${inp(`spellcasting.groups.${gi}.slots`,g.slots||0,'number')}<label>Prep.</label>${inp(`spellcasting.groups.${gi}.prepared`,g.prepared||0,'number')}<label>Usati</label>${inp(`spellcasting.groups.${gi}.used`,g.used||0,'number')}<button class="row-del" data-del="spellcasting.groups.${gi}" title="Elimina cerchia">×</button></span><button class="mini-add" data-add="spellcasting.groups.${gi}.spells" data-kind="spell" hidden>+ Spell</button></summary><div class="spell-slots-read"><span>${esc(g.className||'Classe non indicata')}</span><span>Slot ${slots}</span><span>Preparati ${g.prepared||0}</span><span class="slot-used-control" onclick="event.stopPropagation()"><button type="button" class="slot-step" data-slot-step="spellcasting.groups.${gi}.used" data-step="-1" aria-label="Togli slot usato">−</button><strong>Usati ${used}</strong><button type="button" class="slot-step" data-slot-step="spellcasting.groups.${gi}.used" data-step="1" aria-label="Aggiungi slot usato">+</button></span></div><table class="sheet-table"><thead><tr><th>Nome</th><th>Scuola</th><th>Tempo di lancio</th><th>Bersaglio</th><th>Durata</th><th class="edit-only">Descrizione</th><th class="edit-only">Azioni</th></tr></thead><tbody>${((g.spells||[]).length?g.spells:[]).map((x,i)=>`<tr class="spell-row info-row" tabindex="0" data-tip="${esc(tipText(x))}"><td>${inp(`spellcasting.groups.${gi}.spells.${i}.name`,x.name||'')}</td><td>${inp(`spellcasting.groups.${gi}.spells.${i}.school`,x.school||'')}</td><td>${inp(`spellcasting.groups.${gi}.spells.${i}.castingTime`,x.castingTime||x.time||'')}</td><td>${inp(`spellcasting.groups.${gi}.spells.${i}.target`,x.target||'')}</td><td>${inp(`spellcasting.groups.${gi}.spells.${i}.duration`,x.duration||'')}</td><td class="edit-only">${area(`spellcasting.groups.${gi}.spells.${i}.description`,x.description||'','edit-description')}</td><td class="edit-only"><button class="row-del" data-del="spellcasting.groups.${gi}.spells.${i}">×</button></td></tr>`).join('')||`<tr><td colspan="7" class="empty-row">Nessun incantesimo inserito.</td></tr>`}</tbody></table></details>`}).join('')||`<div class="empty-row spell-empty">Nessuna cerchia inserita. Entra in modifica e aggiungi la prima cerchia utile al personaggio.</div>`}</div>`;return section('Incantesimi / Poteri',body,{wide:true})}
...
function classLevelsPanel(d){let body=`<div class="section-head"><button class="mini-add" data-add="classLevels" data-kind="classLevel" hidden>+ Classe / CdP</button></div><p class="section-mini-note">Il livello totale si calcola automaticamente dalla somma delle classi e classi di prestigio inserite qui.</p><table class="sheet-table"><thead><tr><th>Classe / CdP</th><th>Livello</th><th>Note</th><th class="edit-only">Azioni</th></tr></thead><tbody>${(d.classLevels||[]).map((c,i)=>`<tr><td>${inp(`classLevels.${i}.name`,c.name||'')}</td><td>${inp(`classLevels.${i}.level`,c.level||1,'number')}</td><td>${inp(`classLevels.${i}.notes`,c.notes||'')}</td><td class="edit-only"><button class="row-del" data-del="classLevels.${i}">×</button></td></tr>`).join('')}</tbody></table>`;return section('Classi e livelli',body,{wide:true,open:true})}
...
function render(data,xpData,compendium){const d=normalize(data);d.xpInfo=xpCalc(d,xpData);window.__thalorCurrentData=d;window.__thalorCompendium=compendium||window.__thalorCompendium||{};document.title=`Scheda — ${d.identity?.name||slug}`;document.body.className=`dynamic-sheet-body theme-${d.meta?.theme||'default'}`;let left=`<aside class="panel dynamic-portrait sheet-theme-panel"><img src="${esc(d.portrait?.image||'')}" alt="${esc(d.portrait?.alt||'Ritratto')}" loading="lazy"><blockquote>${esc(d.portrait?.quote||'')}</blockquote><div class="portrait-edit-tools edit-only"><label class="portrait-upload-label">Cambia foto<input id="portraitImageUpload" type="file" accept="image/*"></label><small>La foto viene compressa per non appesantire il salvataggio locale.</small></div><h2>Identità</h2>${statBlock(d.identity,'identity',[['player','Giocatore'],['race','Razza'],['alignment','Allineamento'],['deity','Divinità'],['xp','PX']])}<div class="sheet-stat identity-level-read"><span>Classe/Livello</span>${readonly(classSummary(d))}</div><div class="sheet-stat identity-level-read"><span>Livello effettivo</span>${readonly(d.xpInfo?.level||totalLevel(d))}</div><h2>Aspetto</h2>${statBlock(d.appearance,'appearance',[['size','Taglia'],['age','Età'],['sex','Sesso'],['height','Altezza'],['weight','Peso'],['eyes','Occhi'],['hair','Capelli'],['skin','Pelle'],['marks','Segni']])}</aside>`;app.innerHTML=hero(d)+`<section class="dynamic-sheet-layout">${left}<div class="dynamic-stack">${xpPanel(d)}${classLevelsPanel(d)}${abilities(d)}${combat(d)}${saves(d)}${attacks(d)}</div></section><section class="dynamic-grid">${conditionsPanel(d)}${defenses(d)}${skills(d)}${tooltipCard('Talenti','feats',d.feats,'feat')}${tooltipCard('Capacità / Tratti','features',d.features,'feature')}${languages(d)}${spells(d)}${inventory(d)}${companionsPanel(d)}${narrative(d)}${historyPanel(d)}${secretsPanel(d)}</section>`;bind(d,xpData,window.__thalorCompendium)}
function enable(on){if(on&&!sheetCanEdit()){alert(editDeniedMessage());on=false;}document.querySelectorAll('[data-path]').forEach(el=>{if(el.classList.contains('condition-active-control'))el.disabled=false;else el.disabled=!on});document.querySelectorAll('.mini-add').forEach(b=>{if(b.classList.contains('always-visible'))b.hidden=false;else b.hidden=!on});const saveBtn=document.getElementById('saveSheet'),editBtn=document.getElementById('editSheet');if(saveBtn)saveBtn.hidden=!on;if(editBtn)editBtn.hidden=on;app.classList.toggle('editing',on);document.body.classList.toggle('editing',on);document.querySelectorAll('.section-controls').forEach(ctrl=>{const e=ctrl.querySelector('.section-edit-btn'),sv=ctrl.querySelector('.section-save-btn');if(e)e.hidden=on||!sheetCanEdit();if(sv)sv.hidden=!on;});}
...
async function saveCurrentSheet(data,xpData,detail,fromDom=true,keepEdit=null){
  if(!sheetCanEdit()){alert(editDeniedMessage());return normalize(fromDom?collect(data):data);}
  ...
  let u=normalize(fromDom?collect(data):data);
...
function bind(data,xpData,compendium){
  document.getElementById('editSheet').onclick=()=>enable(true);
  document.getElementById('saveSheet').onclick=()=>{saveCurrentSheet(data,xpData,'Salvataggio completo dalla barra superiore.');enable(false);document.getElementById('localStatus').textContent='Modifiche salvate nel browser.'};
  document.getElementById('resetSheet').onclick=()=>{if(!sheetCanEdit()){alert(editDeniedMessage());return;}if(isCompanion){alert('Questa è una scheda secondaria: per eliminarla torna alla scheda principale e usa la X sulla card. Per azzerarla puoi importare un JSON vuoto/template.');return;}localStorage.removeItem(storageKey);oldKeys.forEach(k=>localStorage.removeItem(k));location.reload()};
  document.getElementById('exportSheet').onclick=()=>download(`thalor-${slug}${isCompanion?'-creatura-'+companionIndex:''}-scheda.json`,JSON.stringify(normalize(collect(data)),null,2));
  document.getElementById('copySheet').onclick=async()=>{await navigator.clipboard.writeText(JSON.stringify(normalize(collect(data)),null,2));document.getElementById('localStatus').textContent='Backup JSON copiato negli appunti.'};
  document.getElementById('exportCompendium').onclick=()=>download('thalor-compendio-locale.json',JSON.stringify(mergeCompendium(window.__thalorCompendium||{}),null,2));
  document.getElementById('printSheet').onclick=()=>{document.querySelectorAll('details').forEach(d=>d.open=true);window.print();};
  document.getElementById('importSheet').onchange=e=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=()=>{try{const u=normalize(JSON.parse(r.result));u.changeLog=u.changeLog||[];u.changeLog.push({when:new Date().toLocaleString('it-IT'),action:'Import JSON',detail:'Scheda importata manualmente.'});saveCurrentSheet(u,xpData,'Scheda importata manualmente.',false,false);enable(false);document.getElementById('localStatus').textContent='Scheda importata e salvata nel browser.'}catch(err){alert('JSON non valido')}};r.readAsText(f)};
  ...
  const quickAddCondition=document.getElementById('quickAddCondition');if(quickAddCondition)quickAddCondition.onclick=e=>{e.preventDefault();e.stopPropagation();let c=normalize(collect(data));let name=(document.getElementById('quickConditionName')?.value||'Custom').trim()||'Custom';c.conditions=c.conditions||[];c.conditions.push(Object.assign({},blank.condition,{name,active:'Sì'}));let saved=saveCurrentSheet(c,xpData,'Condizione aggiunta: '+name+'.',false);document.getElementById('localStatus').textContent='Condizione aggiunta: '+name+'.';};
  ...
  document.querySelectorAll('.section-edit-btn').forEach(btn=>btn.onclick=e=>{e.preventDefault();e.stopPropagation();if(!sheetCanEdit()){alert(editDeniedMessage());return;}const sec=nearestSection(btn);setSectionEdit(sec,true);btn.hidden=true;const save=sec?.querySelector('.section-save-btn');if(save)save.hidden=false;document.getElementById('localStatus').textContent='Modifica sezione: '+(btn.dataset.sectionTitle||'sezione')+'.'});
  document.querySelectorAll('.section-save-btn').forEach(btn=>btn.onclick=e=>{
    e.preventDefault();e.stopPropagation();
    const title=btn.dataset.sectionTitle||'sezione';
    const globalEdit=app.classList.contains('editing');
    const current=normalize(collect(data));
    saveCurrentSheet(current,xpData,'Salvataggio sezione: '+title+'.',false,globalEdit);
    if(globalEdit) enable(true);
    document.getElementById('localStatus').textContent='Sezione salvata: '+title+'.';
  });
