...
function localStorageHint(){return `Copia locale: browser di questo dispositivo → localStorage → chiave ${storageKey}. Non è un file scaricato.`;}
function setLocalStatus(text){const ls=document.getElementById('localStatus'); if(ls)ls.textContent=text||localStorageHint();}
...
function identityProgressPanel(d){const x=d.xpInfo;if(!x)return '';const pct=Math.max(0,Math.min(100,Number(x.progress)||0));return `<div class="identity-progress-card"><div class="identity-progress-top"><div class="sheet-stat identity-xp-edit"><span>Esperienza</span>${inp('identity.xp',d.identity?.xp||0,'number')}</div><div class="identity-level-badge"><span>Livello</span><strong>${x.level}</strong></div></div><div class="xpbar compact" title="${esc(x.progressLabel)} verso il prossimo livello" style="--xp:${pct}%"><i style="width:${pct}%"></i><span>${esc(x.progressLabel)}</span></div><div class="identity-progress-meta"><span>Mancano ${esc(x.missing)}</span><span>Prossimo livello ${esc(x.nextLevel)}</span></div>${x.canLevel?`<p class="levelup-warning compact-warning">PX sufficienti per il livello ${x.xpLevel}.</p>`:''}</div>`}
function abilities(d){let gb=globalBonusTotals(d);return `<article class="panel sheet-theme-panel abilities-panel"><h2>Caratteristiche</h2><p class="section-mini-note">I bonus avanzati sotto ogni caratteristica vengono sommati come layer separato. I bonus dello stesso tipo non si sommano, salvo tipi cumulabili.</p><div class="ability-grid">${AB.map(([k,l])=>{let a=d.abilities?.[k]||{score:10,temp:0},baseMod=mod(a.score,a.temp),eff=abilityScoreEffective(d,k),effMod=abilityMod(d,k);return `<div class="ability-card"><span>${k}</span><strong>${inp(`abilities.${k}.score`,a.score,'number')}</strong><em>${readonly(sign(effMod))}</em><small>${l}</small><small>Effettivo: ${eff} · Bonus tipizzati: ${sign(gb[k]||0)}</small>${bonusSummary(a.bonuses||[],k)}${bonusRows(`abilities.${k}.bonuses`,a.bonuses||[])}</div>`}).join('')}</div></article>`}
...
function skills(d){let c=calc(d),lvl=levelFrom(d);let body=`<div class="section-head"><button class="mini-add" data-add="skills" data-kind="skill" hidden>+ Abilità</button></div><p class="section-mini-note">Totale: gradi + caratteristica + mod vari + penalità armatura automatica + condizioni. Max gradi: classe ${lvl+3}, incrociata ${(lvl+3)/2}.</p><table class="sheet-table skill-table"><thead><tr>${dragTh()}<th>Abilità</th><th>Totale</th><th>Car.</th><th>Gradi</th><th>Max</th><th>Mod vari</th><th>Pen. arm.</th><th>Cond.</th><th>Classe</th><th>Note</th><th class="edit-only">Azioni</th></tr></thead><tbody>${(d.skills||[]).map((s,i)=>{let applies=s.armorApplies===true||String(s.armorApplies)==='Sì';let pen=applies?c.pt.armorPenalty*(String(s.name).toLowerCase()==='nuotare'?2:1):0;let condSkill=conditionSkillMod(d,s);let total=num(s.ranks)+abilityMod(d,s.ability)+num(s.misc)+pen+condSkill;let isClass=String(s.classSkill).toLowerCase().startsWith('s');let max=isClass?lvl+3:(lvl+3)/2;let warn=num(s.ranks)>max?' rank-warn':'';return `<tr class="${warn}"${dragDropAttrs(`skills.${i}`)}>${dragTd(`skills.${i}`)}<td>${inp(`skills.${i}.name`,s.name)}</td><td>${readonly(sign(total))}</td><td>${sel(`skills.${i}.ability`,s.ability,s.standardAbility)}</td><td>${inp(`skills.${i}.ranks`,s.ranks,'number')}</td><td>${readonly(max)}</td><td>${inp(`skills.${i}.misc`,s.misc,'number')}</td><td>${readonly(sign(pen))}<span class="edit-only-inline">${yesNo(`skills.${i}.armorApplies`,applies?'Sì':'No')}</span></td><td>${readonly(sign(condSkill))}</td><td>${yesNo(`skills.${i}.classSkill`,isClass?'Sì':'No')}</td><td>${inp(`skills.${i}.notes`,s.notes)}</td><td class="edit-only"><button class="row-del" data-del="skills.${i}">×</button></td></tr>`}).join('')}</tbody></table>`;return section('Abilità',body,{wide:true})}
...
function languages(d){let body=`<div class="section-head"><button class="mini-add" data-add="languages" data-kind="language" hidden>+ Linguaggio</button></div><div class="wa-cards compact language-cards">${(d.languages&&d.languages.length?d.languages:[blank.language]).map((x,i)=>`<div class="wa-card"${dragDropAttrs(`languages.${i}`)}>${dragHandle(`languages.${i}`)}<h3>${inp(`languages.${i}.name`,x.name||'')}</h3>${inp(`languages.${i}.notes`,x.notes||'')}<button class="row-del edit-only" data-del="languages.${i}">×</button></div>`).join('')}</div>`;return section('Linguaggi',body)}
...
function classLevelsPanel(d){let list=(d.classLevels&&d.classLevels.length?d.classLevels:[blank.classLevel]);let body=`<div class="section-head identity-section-tools"><button class="mini-add" data-add="classLevels" data-kind="classLevel" hidden>+ Classe / CdP</button></div><div class="identity-class-list">${list.map((c,i)=>`<article class="identity-class-card"${dragDropAttrs(`classLevels.${i}`)}>${dragHandle(`classLevels.${i}`)}<div class="identity-class-name">${inp(`classLevels.${i}.name`,c.name||'')}</div><div class="identity-class-level"><span>Lv</span>${inp(`classLevels.${i}.level`,c.level||1,'number')}</div><div class="identity-class-notes">${inp(`classLevels.${i}.notes`,c.notes||'')}</div><button class="row-del edit-only" data-del="classLevels.${i}">×</button></article>`).join('')}</div>`;return section('Classi e livelli',body,{open:true})}
...
function render(data,xpData,compendium){const d=normalize(data);const can=sheetCanEdit();app.classList.toggle('no-edit-permission',!can);d.xpInfo=xpCalc(d,xpData);window.__thalorCurrentData=d;window.__thalorCompendium=compendium||window.__thalorCompendium||{};document.title=`Scheda — ${d.identity?.name||slug}`;document.body.className=`dynamic-sheet-body theme-${d.meta?.theme||'default'}`;let left=`<aside class="panel dynamic-portrait sheet-theme-panel"><img src="${esc(d.portrait?.image||'')}" alt="${esc(d.portrait?.alt||'Ritratto')}" loading="lazy"><blockquote>${esc(d.portrait?.quote||'')}</blockquote><div class="portrait-edit-tools edit-only"><label class="portrait-upload-label">Cambia foto<input id="portraitImageUpload" type="file" accept="image/*"></label><small>La foto viene compressa per non appesantire il salvataggio locale.</small></div><h2>Identità</h2>${identityProgressPanel(d)}${classLevelsPanel(d)}${statBlock(d.identity,'identity',[['player','Giocatore'],['race','Razza'],['alignment','Allineamento'],['deity','Divinità']])}${languages(d)}<h2>Aspetto</h2>${statBlock(d.appearance,'appearance',[['size','Taglia'],['age','Età'],['sex','Sesso'],['height','Altezza'],['weight','Peso'],['eyes','Occhi'],['hair','Capelli'],['skin','Pelle'],['marks','Segni']])}</aside>`;app.innerHTML=hero(d)+floatingActions()+`<section class="dynamic-sheet-layout">${left}<div class="dynamic-stack">${abilities(d)}${combat(d)}${saves(d)}${attacks(d)}</div></section><section class="dynamic-grid">${conditionsPanel(d)}${defenses(d)}${skills(d)}${tooltipCard('Talenti','feats',d.feats,'feat')}${tooltipCard('Capacità / Tratti','features',d.features,'feature')}${spells(d)}${inventory(d)}${companionsPanel(d)}${narrative(d)}${historyPanel(d)}${secretsPanel(d)}</section>`;bind(d,xpData,window.__thalorCompendium)}
...
async function saveCurrentSheet(data,xpData,detail,fromDom=true,keepEdit=null){
  ...
  if(!await refreshEditPermission()){
    ...
    setLocalStatus('Sessione non valida: copia locale/emergenza salvata, ma non pubblicata online.');
    return draft;
  }
  ...
  try{
    if(authAvailable() && window.ThalorAuth.state.configured && !window.ThalorAuth.state.localMaster){
      ...
      setLocalStatus('Modifiche salvate online. Backup locale aggiornato in questo browser.');
    }else{
      setLocalStatus(authAvailable()&&window.ThalorAuth.state.localMaster?'Modifiche salvate in locale come Master offline.':'Modifiche salvate nel browser.');
    }
  }catch(err){
