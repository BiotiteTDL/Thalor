...
// Permessi UI: in sola lettura mostra solo "Torna al profilo/scheda principale".
(function(){
...
function bind(data,xpData,compendium){
  const editSheetBtn=document.getElementById('editSheet'); if(editSheetBtn) editSheetBtn.onclick=()=>enable(true);
  const saveSheetBtn=document.getElementById('saveSheet'); if(saveSheetBtn) saveSheetBtn.onclick=()=>{saveCurrentSheet(data,xpData,'Salvataggio completo dalla barra superiore.');enable(false);document.getElementById('localStatus').textContent='Modifiche salvate nel browser.'};
  ...
  const printSheetBtn=document.getElementById('printSheet'); if(printSheetBtn) printSheetBtn.onclick=()=>{document.querySelectorAll('details').forEach(d=>d.open=true);window.print();};
