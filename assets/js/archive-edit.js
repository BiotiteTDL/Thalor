...
function renderDocuments(data,edit){
  ...
  const categories=data.categories.map((cat,ci)=>`<details class="archive-edit-category" ${ci===0?'open':''} data-cat="${ci}"><summary><span class="${edit?'editable-field':''}" ${edit?field(`cat.${ci}.title`):''}>${esc(cat.title)}</span><small>${cat.items.length} elementi</small></summary><div class="doc-list archive-edit-list">${cat.items.map((it,ii)=>`<div class="doc-row has-preview archive-edit-item" data-item="${ci}.${ii}">${edit?`<div class="archive-upload-box">${imgPreview(it.image,it.title,it.badge)}<label class="archive-upload-button">Cambia immagine<input type="file" accept="image/*" data-image-input="${ci}.${ii}"></label><input class="archive-url-input" type="text" value="${esc(it.href)}" ${field(`item.${ci}.${ii}.href`,true)} placeholder="Link/file"></div><div class="archive-edit-text"><strong class="editable-field" ${field(`item.${ci}.${ii}.title`)}>${esc(it.title)}</strong><span class="editable-field" ${field(`item.${ci}.${ii}.subtitle`)}>${esc(it.subtitle)}</span><span class="editable-field archive-badge-edit" ${field(`item.${ci}.${ii}.badge`)}>${esc(it.badge||'')}</span></div><button class="row-del archive-delete-item" type="button" data-delete-item="${ci}.${ii}">×</button>`:`<a class="doc-row-link" href="${esc(it.href||it.image||'#')}" target="_blank" rel="noopener">${imgPreview(it.image,it.title,it.badge)}<strong>${esc(it.title)}</strong><span>${esc(it.subtitle)}</span></a>`}</div>`).join('')}</div>${edit?`<div class="archive-inline-actions"><button class="button ghost-button" type="button" data-add-item="${ci}">+ Documento</button><button class="button ghost-button danger-action" type="button" data-delete-category="${ci}">Elimina categoria</button></div>`:''}</details>`).join('');
  app.innerHTML=`<section><h2 class="section-title ${edit?'editable-field':''}" ${edit?field('title'):''}>${esc(data.title)}</h2><p class="section-note ${edit?'editable-field':''}" ${edit?field('note'):''}>${esc(data.note)}</p><div class="doc-accordion archive-edit-accordion">${categories}</div><p style="margin-top:34px;text-align:center"><a class="button" href="../archivio.html">Torna all'archivio</a></p></section><footer>Thalor</footer>${can?floatingActions(edit):''}`;
}
function renderSymbols(data,edit){
  const can=canMasterEdit(); app.classList.toggle('archive-editing',!!edit); app.classList.toggle('archive-readonly',!can);
  const categories=data.categories.map((cat,ci)=>`<details class="archive-edit-category symbol-edit-category" ${ci===0?'open':''} data-cat="${ci}"><summary><span class="${edit?'editable-field':''}" ${edit?field(`cat.${ci}.title`):''}>${esc(cat.title)}</span><small>${cat.items.length} elementi</small></summary><div class="symbols archive-symbol-grid">${cat.items.map((it,ii)=>`<article class="panel symbol-card archive-edit-item" data-item="${ci}.${ii}">${edit?`<div class="archive-upload-box">${imgPreview(it.image,it.title)}<label class="archive-upload-button">Cambia immagine<input type="file" accept="image/*" data-image-input="${ci}.${ii}"></label><input class="archive-url-input" type="text" value="${esc(it.href)}" ${field(`item.${ci}.${ii}.href`,true)} placeholder="Link pagina"></div><div><span class="tag editable-field" ${field(`item.${ci}.${ii}.tag`)}>${esc(it.tag)}</span><h3 class="editable-field" ${field(`item.${ci}.${ii}.title`)}>${esc(it.title)}</h3><textarea class="archive-edit-textarea" ${field(`item.${ci}.${ii}.description`,true)}>${esc(it.description)}</textarea></div><button class="row-del archive-delete-item" type="button" data-delete-item="${ci}.${ii}">×</button>`:`<img alt="${esc(it.title)}" src="${esc(it.image)}" loading="lazy" decoding="async"><div><span class="tag"><a class="lore-link" href="${esc(it.href||'#')}">${esc(it.tag)}</a></span><h3>${esc(it.title)}</h3><p>${esc(it.description)}</p></div>`}</article>`).join('')}</div>${edit?`<div class="archive-inline-actions"><button class="button ghost-button" type="button" data-add-item="${ci}">+ Simbolo</button><button class="button ghost-button danger-action" type="button" data-delete-category="${ci}">Elimina categoria</button></div>`:''}</details>`).join('');
  app.innerHTML=`<section><h2 class="section-title ${edit?'editable-field':''}" ${edit?field('title'):''}>${esc(data.title)}</h2><p class="section-note ${edit?'editable-field':''}" ${edit?field('note'):''}>${esc(data.note)}</p><div class="archive-symbol-categories">${categories}</div><p style="margin-top:34px"><a class="button" href="../archivio.html">Torna all'archivio</a></p></section><footer>Thalor</footer>${can?floatingActions(edit):''}`;
}
...
function bind(data){
  ...
  app.querySelectorAll('[data-delete-category]').forEach(btn=>btn.onclick=()=>{const d=collect(data); d.categories.splice(Number(btn.dataset.deleteCategory)||0,1); render(d,true);});
