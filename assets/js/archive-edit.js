...
(async function start(){let data=type==='documents'?baseDocuments():baseSymbols();let freshLoaded=false; try{if(authAvailable()&&window.ThalorAuth.state&&window.ThalorAuth.state.configured&&navigator.onLine!==false){const online=await window.ThalorAuth.loadCharacter(slug,null,{publicRead:true,skipInit:true}); if(online&&typeof online==='object'){data=normalize(online);freshLoaded=true; localStorage.setItem(storageKey,JSON.stringify(data));}}}catch(e){console.warn('Archivio load online:',e);} if(!freshLoaded){try{const local=localStorage.getItem(storageKey); if(local)data=normalize(JSON.parse(local));}catch(e){}} render(data,false);})();
})();
